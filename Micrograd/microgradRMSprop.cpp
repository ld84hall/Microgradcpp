...
int main(){
	...
	vector<double> v(n.parameters().size(), 0.0);
	...
	//		n.parameters()[j]->data += -0.1 * n.parameters()[j]->grad;
	// the above implements stochastic gradient descent.... 
			v[j] = B*v[j] + (1-B)*pow((n.parameters()[j]->grad),2);
		};
		for (int j=0; j<size; j++){
			n.parameters()[j]->data += (-0.05/(pow((v[j]+ e), 0.5)))*(n.parameters()[j]->grad);
		};
